...
<h3>Monthly Revenue By Customer</h3>

<table border="1">
<form action="emp_revenue_mon_customer.jsp" method="post">
			 Start:<br>
...
SELECT C.lname AS 'Last Name', C.fname AS 'First Name', SUM(R.bookingfee + (R.passengers * F.fares) - R.fare_restrictions) AS 'Customer Revenue' 
...
GROUP BY R.login WHERE R.date >= ? AND R.date <= ? 
...
<%
	try{ 
		...
		PreparedStatement statement = connection.prepareStatement("SELECT C.lname AS 'Last Name', C.fname AS 'First Name', SUM(R.bookingfee + R.total_fare) AS 'Customer Revenue' FROM loginschema.reservations R JOIN loginschema.customer C ON R.login = C.user_name  WHERE R.date >= ? AND R.date <= ? GROUP BY R.login ORDER BY (R.bookingfee + R.total_fare) DESC;");
