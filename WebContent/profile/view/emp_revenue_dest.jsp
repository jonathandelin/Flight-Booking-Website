...
SELECT F.airport_arrive AS 'Destination City', SUM(R.bookingfee + (R.passengers * F.fares) - R.fare_restrictions) AS 'Total Revenue'
FROM loginschema.reservations R JOIN loginschema.flights F ON R.booked_flightnum = F.flightnum
GROUP BY F.airport_arrive
 --%>
 
<%--
SELECT L.a_airport_id AS 'Destination City', SUM(L.fare * R.passengers) AS 'Destination City Revenue'
...
<%
	try{ 
		...
		String total_revenue_by_dest = ("SELECT L.a_airport_id AS 'Destination City', SUM(L.fare * R.passengers) AS 'Destination City Revenue' FROM loginschema.reservations R JOIN loginschema.reserved_legs RL ON RL.res_numR = R.res_num JOIN loginschema.legs L ON RL.flight_numR = L.flight_num AND RL.departure = L.departure GROUP BY L.a_airport_id ORDER BY SUM(R.passengers*L.fare) DESC;");
		ResultSet resultSet = statement.executeQuery(total_revenue_by_dest);
...
<td><%=resultSet.getString("Destination City Revenue") %></td>
</tr>
